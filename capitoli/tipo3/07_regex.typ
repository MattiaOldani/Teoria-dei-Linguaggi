...
#example()[
  ...
  #table(
    ...
    [$L = {a^(2k + 1) bar.v k gt.eq 0}$], [$a (a a)^* = (a a)^* a$],
...
Andiamo ora a dimostrare il @kleene nella sua forma con le espressioni regolari.
...
#theorem-proof()[
  ...
  La dimostrazione si trova nel @regex-to-automa[Paragrafo].
]
...
#example()[
  Per ricavare una espressione regolare da un automa si può usare un algoritmo di *programmazione dinamica* molto simile all'algoritmo Floyd-Warshall sui grafi, che cerca i cammini minimi imponendo una serie di vincoli.
  ...
  #figure(image("assets/07/regex_esempio.svg"))

  Associamo ad esso un sistema di $3$ equazioni, nel quale indichiamo gli stati con le variabili $X_i$ e i caratteri sono quelli dell'alfabeto ${a,b}$. Il sistema è il seguente: $ cases(X_0 = a X_1 + b X_0, X_1 = a X_2 + b X_0, X_2 = a X_2 + b X_1 + epsilon) . $
  ...
  #figure(image("assets/07/regex_regola.svg"))
...
Partiamo con le tre *espressioni regolari base*, che possiamo riconoscere con gli automi presenti nella @espressioni-base[Tabella].
...
#figure(
  table(
    ...
    [$emptyset.rev$], [#figure(image("assets/07/vuoto.svg"))],
    [$epsilon$], [#figure(image("assets/07/epsilon.svg"))],
    [$a$], [#figure(image("assets/07/singolo_carattere.svg"))],
  ),
)<espressioni-base>
...
#example()[
  ...
  #figure(image("assets/07/l3c_ok01.svg"))

  Una seconda soluzione utilizza invece il non determinismo puro.

  #figure(image("assets/07/l3c_ok02.svg"))
]
...
#example()[
  ...
  #grid(
    ...
    align: center + horizon,
    inset: 10pt,
    [#figure(image("assets/07/a_m.svg"))], [#figure(image("assets/07/b_n.svg"))],
  )

  Costruiamo l'automa prodotto per il linguaggio $L = L' union L''$.

  #figure(image("assets/07/unione_am_bn.svg"))
]
...
Per i *DFA* possiamo utilizzare la costruzione dell'automa prodotto appena definita modificando l'insieme degli stati finali $F$ rendendolo $ F = {(q,p) bar.v q in F' and p in F''} . $
...
#example()[
  ...
  #figure(image("assets/07/intersezione_am_bn.svg"))
]
...
#figure(image("assets/07/prodotto.svg"))
...
Con questa ultima operazione chiudiamo la dimostrazione del @kleene.

Questa operazione la possiamo definire come $ L^* = union.big_(k gt.eq 0) L^k = {w in Sigma^* bar.v exists x_1, dots, x_k in L bar.v k gt.eq 0 bar.v w = x_1 dots x_k} . $

Un automa per la *star* deve cercare di scomporre la stringa in ingresso in più stringhe di $L$. Possiamo prendere spunto dall'automa per la concatenazione: facciamo partire l'automa per $L$ e ogni volta che arriviamo in uno stato finale lo facciamo ripartire dallo stato iniziale. Devo accettare anche la parola vuota, quindi aggiungiamo uno stato iniziale finale.

#figure(image("assets/07/kleene.svg"))
...
#example()[
  ...
  #figure(image("assets/07/star_corretta.svg"))
]<b-marcatore>
...
Come solito, l'aggiunta di non determinismo agli NFA a noi non cambia, anzi ci piace, anche perché in questo caso l'automa lo otteniamo in modo semplice aggiungendo solo uno stato, ovvero $ nsc(L^*) lt.eq nsc(L) + 1 . $

== Codici

Abbiamo visto il linguaggio dell'@b-marcatore, che era estremamente comodo da scomporre per via della presenza di una sola $b$ nella stringa in input. Vediamo ora qualche altro esempio notevole.

#example()[
  Definiamo il linguaggio $L = a a a b a^*$. Vogliamo calcolare $L^*$.
...
#example()[
  Definiamo ora il linguaggio $L = a (b + b a a b) a^*$. Vogliamo calcolare $L^*$.
...
#example()[
  ...
  #figure(image("assets/07/estese_NFA.svg"))
  ...
  #figure(image("assets/07/estese_DFA.svg"))
  ...
  #figure(image("assets/07/estese_complemento.svg"))
